...
try:
#    VLs_USA = "E:/코딩/codess/MyWorks/TankCombatSz/TankCombat_alpha/TankCombat_alpha-test_ver_3/VLs/USA"
    VLs_USAs = []
    for file in os.listdir(VLs_USA):
        if file.endswith(".mp3"):
            VLs_USAs.append(os.path.join(VLs_USA, file))
except:
    pass
...
Fls_s = 0
Fls_d = 100
Fls_cd = int(Cntr_L[SltdCntrIdx][SltdTimeIdx][SltdTankIdx].MG_I_List[1] * 1000)
LFT = - Fls_cd
#VLs_USAs 0 = USA_APCR | 1 = USA_Cps | 2 = USA_HE | 3 = USA_AP | 4 = USA_Fire1 | 5 = USA_Fire2
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_1:
                amm = "AP(철갑탄)"
                if Rdd_APs != 1:                   
                    pygame.mixer.music.load(VLs_USAs[3])
            ...
            if event.key == pygame.K_3:
                amm = "HE(고폭탄)"
                if Rdd_HEs != 1:                   
                    pygame.mixer.music.load(VLs_USAs[2])
                    pygame.mixer.music.play()
                    Rdd_HEs = 1
                Rdd_APs = 0
            if event.key == pygame.K_SPACE:
                if now - LFT >= Fls_cd:
                    ...
                    Fls_s = now
                    LFT = now
#                    fr = random.randrange(4,6)
#                    pygame.mixer.music.load(VLs_USAs[fr])
#                    pygame.mixer.music.play()
                
    if Fls:
        now = pygame.time.get_ticks()
        if now - Fls_s < Fls_d:
            pygame.draw.circle(Dsp, (255, 255, 0), (x, y), 400)
            fr = random.randrange(4,6)
            pygame.mixer.music.load(VLs_USAs[fr])
            pygame.mixer.music.play()
        else:
    ...
    text_surf = font.render(amm, True, Blc)
    ...
    pygame.draw.rect(Dsp, Blc, (x, 300, 300, 50))
